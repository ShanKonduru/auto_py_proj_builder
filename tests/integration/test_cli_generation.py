...
class TestCLIProjectGeneration:
    ...
    def test_cli_template_requirements(self):
        ...
        result = self.runner.invoke(
            main,
            ["generate", project_name, "--template", "cli", "--output-dir", str(self.temp_path)],
        )
    ...
    def test_cli_template_setup_py_entry_point(self):
        ...
        pyproject_toml_path = self.temp_path / project_name / "pyproject.toml"
        assert pyproject_toml_path.exists()

        content = pyproject_toml_path.read_text()
        # Should include console_scripts entry point in pyproject.toml
    ...
    def test_python_version_setting(self):
        ...
        # Python version should be reflected in pyproject.toml
        assert (
            f'requires-python = ">={python_version}"' in content
            or f"python_requires = \">={python_version}\"" in content
            or python_version in content
        )
    ...
    def test_cli_project_name_as_package(self):
        ...
        # Should create main.py as the CLI entry point
        main_path = self.temp_path / project_name / "main.py"
        assert main_path.exists()

        # Check if main.py contains CLI structure
        main_content = main_path.read_text()
        assert "click" in main_content.lower() or "cli" in main_content.lower()
    ...
    def test_author_and_email_in_generated_files(self):
        ...
        # Check pyproject.toml contains author info
        pyproject_toml_path = self.temp_path / project_name / "pyproject.toml"
        pyproject_content = pyproject_toml_path.read_text()
        assert author_name in pyproject_content
