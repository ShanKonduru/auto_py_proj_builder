...
@click.option(
    "--name", "-n", "name_option", help="Project name (can also be provided as first argument)"
)
@click.option(
    "--type",
    "-t",
...
@click.option("--python-version", default="3.9", help="Minimum Python version (default: 3.9)")
@click.option("--dependencies", help="Comma-separated list of additional dependencies")
@click.option(
    "--include-tests/--no-tests", default=True, help="Include test configuration (default: yes)"
)
@click.option(
    "--include-linting/--no-linting",
    default=True,
    help="Include linting configuration (default: yes)",
)
@click.option(
    "--coverage-threshold", type=int, default=50, help="Coverage fail-under threshold (default: 50)"
)
@click.option(
    "--include-batch-files/--no-batch-files",
    default=True,
    ...
    try:
        ...
        if not project_name:
            raise click.BadParameter(
                "Project name is required (use first argument or --name option)"
            )
        ...
        _validate_inputs(
            validator, project_name, author, email, python_version, coverage_threshold, dependencies
        )
        ...
        configuration_profile = ConfigurationProfile(profile_type=profile_type)

        # Update coverage settings if needed
        if include_tests and coverage_threshold != 50:
            configuration_profile.coverage_settings["fail_under"] = coverage_threshold

        # Show generation plan
        if verbose or dry_run:
            _show_generation_plan(
                project_template, project_metadata, configuration_profile, target_path
            )
...
def _validate_inputs(
    ...
    if not validator.is_valid_python_version(python_version):
        raise click.BadParameter("Python version must be in format X.Y (e.g., 3.9, 3.10)")

    # Validate coverage threshold
    if not (0 <= coverage_threshold <= 100):
        raise click.BadParameter("Coverage threshold must be between 0 and 100")

    # Validate dependencies format
    if dependencies and not validator.is_valid_dependency_list(dependencies):
        raise click.BadParameter("Dependencies must be comma-separated package names")
...
def _get_default_patterns(template_type: TemplateType) -> list:
    ...
    base_patterns = ["*.py", "*.md", "*.txt", "*.ini", "*.toml", "*.cfg"]
...
def _show_generation_plan(
    ...
    click.echo(
        f"  Author: {project_metadata.author_info['name']} <{project_metadata.author_info['email']}>"
    )
